...
- Join tạm 5 bảng trong RAM (không dùng `returns.csv`) để phục vụ vẽ biểu đồ và phân tích.
- Loại bỏ duplicate trong `order_items` theo tổ hợp `(order_id, product_id)` trước khi join.
- Tạo nhãn `returned_label` (nếu chưa tạo), ép kiểu datetime, tính `customer_tenure_days`.
